...
(function($){
    ...
    var updateGpadTypeMaps = function () {gamepadmaps = $.storage.getObject('gamepadmaps') || {}};
    ...
    var getGamePadInputInfo = function (gpadType, key, value) {
        ...
        if(!refToAxisMap) {
            ...
            var text = getGamePadText(gpadType, 'button', key);
            ...
            var text = getGamePadText(gpadType, 'axis_dual', refToAxisMap.bid);
    ...
    inpoot.tick = function (permission) {
        ...
        for(var i=0; i < gamepads.length; i++){
            ...
            if (rawDog && rawDog.gpadType) {
                ...
                //look at all axis
                for (var j = 0; j < rawDog.axes.length; j++) {
                    var thisGMap = getGPadTypeMapping(rawDog.gpadType, 'axes', j);
                    var absValue = Math.abs(rawDog.axes[j]);
                ...
                function(){
                    ...
                    for(var i=0; i < gamepadsToCheck.length; i++){
                        ...
                        if (rawDog) {
                            ...
                            //look at all axis
                            for (var j = 0; j < rawDog.axes.length; j++) {
                                if (Math.abs(rawDog.axes[j]) > 0.7 && !buttonPressed) {
                                    buttonPressed = true;
                                    inpoot.utils.poll.clear('single-button-press');
                                    callBack({
                                        type: 'axis',
        ...
        var gpadPP = function (item) {
            ...
            inpoot.utils.poll.setTimeout(function(){
                ...
                for(var button in padConfigs.button){
                    ...
                    var face = thisButton.face || "front";
        ...
        var refreshPads = function () {
            ...
            for(var i=0; i < gamepads.length; i++){
                ...
                for(var j=0; j < newList.length; j++){
                    if(newList[j].id == gamepads[i].gamepad.id){
                ...
                if(!found){
                    newList.push({
                        id: gamepads[i].gamepad.id,
                        displayName: gamepads[i].gamepad.displayName,
                        active: true,
                        gpadType: gamepads[i].gamepad.gpadType
                    });
                    storedGamepads.push({
                        id: gamepads[i].gamepad.id,
                        displayName: gamepads[i].gamepad.displayName,
                ...
                for(var i=0; i < newList.length; i++){
                    for(var j=0; j < oldList.length; j++){
                         if(oldList[j].id == newList[i].id){
                             if(oldList[j].active == newList[i].active){
                                 matches++;
                             }   
                         }
                    }
    ...
    var launch_players = function () {
        ...
        var updatePlayer = function (playerNum, playerObj, callBack) {
            ...
            for (j = 0; j < storedPlayers.length; j++) {
        ...
        var eachPlayerPP = function (item) {
            ...
            if (item.data.gamepad) {
                ...
                if(item.data.options.gpadIndex !== false && gpadList[item.data.options.gpadIndex] && gpadList[item.data.options.gpadIndex].gpadType == item.data.gamepad){
                    uiNode.find('.inpoot-player-gamepad-index').html('Gamepad Slot ' + (parseInt(item.data.options.gpadIndex) + 1));
                } else {
    ...
    inpoot.utils = {
        ...
        poll : {
            ...
            setTimeout: function (callBack, timeAmount, pollId) {
                var pollId = pollId || this.getPollId();
                var thisPoll = {
                    type:'timeout',
